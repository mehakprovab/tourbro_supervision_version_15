...
@Component({
  selector: 'app-transfer-booking-payment-status',
  templateUrl: './transfer-booking-payment-status.component.html',
  styleUrls: ['./transfer-booking-payment-status.component.scss']
})
export class TransferBookingPaymentStatusComponent implements OnInit {
  TransferAppRef: string;
  appReference: any;
  orderId: any;
  booking_source: any;
  ...
  ngOnInit() {
    ...
    const storedData = sessionStorage.getItem("TransferReportAppRef");

    if (storedData) {
      try {
        this.TransferAppRef = storedData;
      } catch (error) {
      ...
      (resp) => {
        if (resp.statusCode == 201 || resp.statusCode == 200) {
          if (resp.data == true) {
            ...
            this.getVoucher(this.TransferAppRef)
          } else {
  ...
  getVoucher(appReference) {
    this.router.navigate(['/reports/transfer-voucher'], { queryParams: { appReference } });
  }
