...
export class CategoriesComponent implements OnInit {
  ...
  ngOnInit(): void {
      ...
      this.subs.sink = this.activityService.clearcategories.subscribe(flag => {
          if(flag) {
              this.clearFilter();
          }
      });
  ...
  createForm(hotels) {
      const amenityCheckboxes = this.activityService.getUniqueAmenities(hotels); // Assuming you have a method to get unique amenities
  ...
  filterByAmenities(isChecked: boolean, amenityName: string) {
      ...
      this.activityService.categories.next(selectedAmenities);

      // Trigger the filtering process in the service
      this.activityService.filterByCategories();
  }


  clearFilter() {
     this.regConfig.reset(); // Reset the form group to its initial state
     this.activityService.categories.next([]);
