...
@Component({
  selector: 'app-recomended-activity',
  templateUrl: './recomended-activity.component.html',
  styleUrls: ['./recomended-activity.component.scss']
})
export class RecomendedActivityComponent implements OnInit {
  regConfig: FormGroup;
  ...
  ngOnInit(): void {
      ...
      this.subs.sink = this.activityService.clearReacomended.subscribe(flag => {
          if(flag) {
              this.clearFilter();
          }
      });
  ...
  createForm(hotels) {
      const amenityCheckboxes = this.activityService.getUniqueRecommanded(hotels); // Assuming you have a method to get unique amenities
  ...
  filterByAmenities(isChecked: boolean, amenityName: string) {
      ...
      this.activityService.reacomended.next(selectedAmenities);

      // Trigger the filtering process in the service
      this.activityService.filterByRecomended();
  }


  clearFilter() {
     this.regConfig.reset(); // Reset the form group to its initial state
     this.activityService.reacomended.next([]);
