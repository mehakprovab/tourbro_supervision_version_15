...
const log = new Logger('FlightCityList');

@Component({
    selector: 'app-flight-city-list',
    templateUrl: './flight-city-list.component.html',
    styleUrls: ['./flight-city-list.component.scss'],
})

export class FlightCityListComponent implements OnInit, OnChanges {
    @Input() getAirports = [];
    @Input() inputFor;
    ...
    onAirportSelect(cityObj: object, inputFor: string): void {
        ...
        if (location) {
            localStorage.setItem('selectedCity', JSON.stringify(cityObj)); // Use localStorage
            console.log('Airports', cityObj);
        } else {
