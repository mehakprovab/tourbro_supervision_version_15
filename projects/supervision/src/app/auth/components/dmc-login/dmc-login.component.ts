...
@Component({
  selector: 'app-dmc-login',
  templateUrl: './dmc-login.component.html',
  styleUrls: ['./dmc-login.component.scss']
})
export class DmcLoginComponent implements OnInit {
  slideConfig2 = {
    className: 'center',
    centerMode: true,
    infinite: true,
...
console.log("this.returnUrl",this.returnUrl)
    ...
    if(this.returnUrl == '/' || this.returnUrl == '/dmc'){
      this.user_type = "dmc"
    }
    this.authService.onLogin(this.loginForm.get('email').value, this.loginForm.get('password').value,this.user_type)
      .subscribe(res => {
        ...
        this.swalService.alert.error('Invalid Credentials');
  ...
  //     .subscribe(res => {
    ...
    .subscribe(res => {
          if (res.statusCode == 200 || res.statusCode == 201) {
              ...
              setTimeout(() => {
                if (this.loginData['auth_role_id'] === 7) {
                  const selectedServices = this.loginData.selectedSuppliers.split(',');
                  ...
                  if (selectedServices.includes('Transfer')) {
                    this.router.navigate(['transfers/transfer-crs-list'],{ queryParams: { tab: 'list_vehicles' } })
                  }
                  if (selectedServices.includes('Tour')) {
                    this.router.navigate(['tour-crs/tour-list'])
                  }
                }
                if (this.loginData['auth_role_id'] === 6) {
                  this.router.navigate(['hotels/hotel-crs-lists'],{ queryParams: { tab: 'list_hotels' } });
                }
              }, 250)
