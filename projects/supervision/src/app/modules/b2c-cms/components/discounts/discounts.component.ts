  ...
  labelImport: ElementRef;
    ...
    discountImage:string;
      ...
      displayColumn: { key: string, value: string }[] = [
        ...
        { key: 'discount_value', value: 'Discount Value' },                
    ...
    createForm(){
       this.regConfig = this.fb.group({
              ...
              discount_value: new FormControl('', [Validators.required,Validators.pattern("[0-9]+(\.[0-9][0-9]?)?")]),
    ...
    getTestimonialList(){
      this.subSunk.sink = this.apiHandlerService.apiHandler('coreDiscountContentList', 'post', {}, {},{})
              .subscribe(resp => {
      ...
      updateStatus(id,status){
        this.subSunk.sink = this.apiHandlerService.apiHandler('updateDiscountContentStatus', 'post', {}, {}, {"id":id,"status":status})
        .subscribe(resp => {
          if (resp && resp.Status && (resp.statusCode === 201 || resp.statusCode === 200)) {
      ...
      editList(patchData){
        this.regConfig.patchValue({
          ...
          discount_type: patchData['discount_type'] || '',
          discount_value: patchData['discount_value'] || '',
          id: patchData['id'] || '',        
          status: patchData['status'] || 0,        
          update_image: patchData['image'] || "",        
        });
        this.btnName = 'Update';
        this.imageSrc= "";
    ...
    onSearchSubmit() {
      if (this.regConfig.invalid) {
         ...
         if(this.fileToUpload)
           req.append('discount_image',this.fileToUpload);
        if(this.regConfig.value.update_image && !this.fileToUpload)
           req.append('discount_image',this.regConfig.value.update_image);
         req.append('title',this.regConfig.value.title);
         req.append('description',this.regConfig.value.description);        
         req.append('discount_type',this.regConfig.value.discount_type);        
         req.append('discount_value',this.regConfig.value.discount_value);        
         let id = this.regConfig.value.id;        

          if(id && id > 0){
            req.append('id',id);
            req.append('status',this.regConfig.value.status);
            this.subSunk.sink = this.apiHandlerService.apiHandler('updateDiscountContent', 'post', {}, {}, req)
            .subscribe(resp => {
              if (resp && resp.Status) {
            ...
            req.append('status',"1");
            this.subSunk.sink = this.apiHandlerService.apiHandler('addDiscountContent', 'post', {}, {}, req)
            .subscribe(resp => {
              if (resp && resp.data) {
      ...
      deletRecord(id){
        this.subSunk.sink = this.apiHandlerService.apiHandler('deleteDiscountContentRecords', 'post', {}, {}, {"id":id})
          .subscribe(resp => {
            if (resp && resp.data) {
