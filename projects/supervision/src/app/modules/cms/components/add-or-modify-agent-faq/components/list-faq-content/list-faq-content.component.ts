...
confirmDelete(id) {
    this.subSunk.sink = this.cmsService.deleteAgentFaq({ "id": id })
        .subscribe(resp => {
...
getContentList() {
    ...
    this.subSunk.sink = this.cmsService.getAgentFaqList(data).subscribe(resp => {
...
onStatusChange(data) {
    console.log(data);
    this.subSunk.sink = this.cmsService.updateAgentFaqListStatus(
