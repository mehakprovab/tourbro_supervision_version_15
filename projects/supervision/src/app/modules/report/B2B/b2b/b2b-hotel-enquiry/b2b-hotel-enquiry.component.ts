...
@Component({
  selector: 'app-b2b-hotel-enquiry',
  templateUrl: './b2b-hotel-enquiry.component.html',
  styleUrls: ['./b2b-hotel-enquiry.component.scss']
})
export class B2bHotelEnquiryComponent implements OnInit {
  ...
  getB2BHotelEnquiry() {
      ...
      this.subSunk.sink = this.apiHandlerService.apiHandler('hotelEnquiryReport', 'post', {}, {},req
          )
          .subscribe(resp => {
              console.log(resp);
  ...
  download(type: any, orientation?: string) {
    ...
    this.config.type = type;
        if (type === 'xlsx' || type === 'xls') {
            this.utility.downloadElementAsExcel(this.config.elementIdOrContent, 'b2b-hotel-enquiry');
            return;
        }
        if (orientation) {
...
downloadPdf() {
    const element = document.getElementById('b2b-hotel-enquiry');
    html2canvas(element).then(canvas => {
        ...
        pdf.save('B2B_Hotel_Enquiry_Report.pdf');
  ...
  openEnquiry(data){
      if(data.status==1){
          ...
          this.subSunk.sink = this.apiHandlerService.apiHandler('hotelEnquiryStatusChange', 'post', {}, {},{
              "status":1,
              "id": data.id
          }
          )
