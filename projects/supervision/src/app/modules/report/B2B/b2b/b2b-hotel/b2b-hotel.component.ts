...
export class B2bHotelComponent implements OnInit {
    ...
    download(type: any, orientation?: string) {
        ...
        if (type === 'xlsx' || type === 'xls') {
            this.utility.downloadElementAsExcel(this.config.elementIdOrContent, 'b2b-hotel');
            return;
        }
        if (orientation) {
    ...
    downloadPdf() {
        this.utility.downloadElementAsPdf('b2b-hotel-report', 'B2B_Hotel_Report', 'landscape');
    }
    ...
    cancelTicketPopup(data) {
        console.log('B2B Stay bundle_booking:', data && data.BookingDetails ? data.BookingDetails.bundle_booking : data && data.bundle_booking, data);
