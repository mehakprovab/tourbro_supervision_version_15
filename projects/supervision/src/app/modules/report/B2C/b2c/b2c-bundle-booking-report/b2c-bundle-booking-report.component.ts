...
@Component({
  selector: 'app-b2c-bundle-booking-report',
  templateUrl: './b2c-bundle-booking-report.component.html',
  styleUrls: ['./b2c-bundle-booking-report.component.scss']
})
export class B2cBundleBookingReportComponent implements OnInit {
  ...
  cancelData: any;
  load: boolean = false;
  @ViewChild('b2c-bundle-report', { static: false }) tableToExport: ElementRef;
  ...
  ngOnInit() {
    ...
    this.getB2cBundleBookingReport();
  }
  ...
  download(type: any, orientation?: string) {
    ...
    this.config.type = type;
        if (type === 'xlsx' || type === 'xls') {
            this.utility.downloadElementAsExcel(this.config.elementIdOrContent, 'b2c-bundle-booking-report');
            return;
        }
        if (orientation) {
...
pdfCallbackFn(pdf: any) {
  ...
  downloadPdf() {
    const element = document.getElementById('b2c-bundle-report');
    html2canvas(element).then(canvas => {
        ...
        pdf.save('B2C_Bundle_Report.pdf');
